...
def test_optimize(optimize):
    ...
    logits, train_op, cross_entropy_loss, accuracy, mean_iou, metrics_op = optimize(layers_output, correct_label, learning_rate, num_classes, global_step)
...
def test_train_nn(train_nn):
    ...
    accuracy = tf.constant(0.91)
    mean_iou = tf.constant(0.11)
    metrics_op = tf.constant(0)
